...
def ConvertCameraToEgo0():
    ...
    for i,row in p.iterrows():
        ...
        x,y,z = np.squeeze(np.array(T_e_e0[frame].reshape(4,4)[:3] *CameraPixel.reshape(4,1)))
        data = data.append({"Pixel": pixel, "X": x, "Y": y, "Z": z,"Frame": frame}, True)
    return data

def calculateCentroid(frame,converted):
    """calculate center of gravity"""
    return (converted.loc[converted['Frame']==frame].sum()/numPixels).to_numpy()[2:5]
...
def printDots(data,frames):
    a = Centroids.loc[(Centroids['Frame']) <50]
...
def calculatYawFromWorld(frame):
    """calculate furthest points to estimate target surface"""
...
def mainFunction():
    ...
    noiseBar = np.percentile(abs(regressions['RSqr']),85);

    skipFrameBar = np.percentile(abs(regressions['RSqr']),5);
    changes=pn.DataFrame()
    for x in np.arange(1,maxFrames):
        """calculate R square and round"""
        Rsqr = round(lineraRegressionFrame(x,convertedEgo0),2)
        ...
        if ((abs(Rsqr) < skipFrameBar)):
            ...
            d_th = calculateHeading(x, framePoints, carLength)
        else:
        ...
        RealHeading = calculatYawFromWorld(x)
